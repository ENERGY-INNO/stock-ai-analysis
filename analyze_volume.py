...
# -*- coding: utf-8 -*-
"""
📊 Quant Analyst Persona: US Stock Volume Analysis
====================================================
Calculates technical indicators for supply/demand analysis:
- OBV (On-Balance Volume)
- A/D Line (Accumulation/Distribution)
- MFI (Money Flow Index)
- Volume Surge Detection

Author: Quant Analyst Persona
Purpose: Identify accumulation/distribution phases through volume analysis
"""
...
class VolumeAnalyzer:
    """
    거래량 기반 기술적 분석을 통한 수급 판단
    
    Indicators:
    - OBV: 가격 방향에 따른 거래량 누적 → 추세 확인
    - A/D Line: 매수/매도 압력 측정 → 스마트머니 추적
    - MFI: 볼륨 가중 RSI → 과매수/과매도 판단
    - Volume Surge: 평균 대비 급등 거래량 → 이벤트 감지
    """
    ...
    def calculate_ad_line(self, df: pd.DataFrame) -> pd.Series:
        ...
        해석: 
        - CLV > 0: 종가가 고가에 가까움 → 매수 우세
        - CLV < 0: 종가가 저가에 가까움 → 매도 우세
        """
    ...
    def analyze_supply_demand(self, df: pd.DataFrame) -> Dict:
        ...
        Returns a dictionary with all indicators and final score
        
        점수 체계:
        - 50점 기준 시작
        - OBV/AD 변화율에 따라 ±15점
        - Volume ratio에 따라 ±10점
        - MFI 상태에 따라 ±5점
        """
        if len(df) < 30:
            return None
